	...
	: TsVNtupleScorer(pM, mM, gM, scM, eM, scorerName, quantity, outFileName, isSubScorer)//,n_count(0)
{
	...
	fNtuple->RegisterColumnD(&fy_z1_Prim, "y_z1_Prim (keV/um)",""); //DEBUG
...
G4bool TsYScorer::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
	...
	if (ParticleName == "e-")       flagParticle = 0;
			...
			&& aStep->GetTrack()->GetParentID()==0)   flagParticle = 1; //DEBUG
	else if (ParticleName == "proton"
			&& aStep->GetTrack()->GetTrackID() !=1
			&& aStep->GetTrack()->GetParentID() !=0)   flagParticle = 2; //DEBUG
...
void TsYScorer::PushBackData(G4double* Edep)
{
	...
	fy_z1_Prim = (Edep[1]/keV)/(fMeanChordLength/um); //DEBUG
...
void TsYScorer::OutputResult()
{
	...
	if(fGetSecondariesContribution)
	{
		...
		fprintf(ySpec_particle,"y(keV/um)    e-           Hprim            Hsec            He           Li           Be           B            C            Other        Total[yd(y)]\n");
