...
#include "MyScoreAmorphusTrack.hh"
#include<vector>
#include<fstream>
...
MyScoreAmorphusTrack::MyScoreAmorphusTrack(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM, TsScoringManager* scM, TsExtensionManager* eM,
...
MyScoreAmorphusTrack::~MyScoreAmorphusTrack() {;}


G4bool MyScoreAmorphusTrack::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
	if (!fIsActive) {
...
void MyScoreAmorphusTrack::AccumulateEvent()
{
...
void MyScoreAmorphusTrack::AbsorbResultsFromWorkerScorer(TsVScorer* workerScorer) {
	TsVBinnedScorer::AbsorbResultsFromWorkerScorer(workerScorer);


	MyScoreAmorphusTrack* workerMTScorer = dynamic_cast<MyScoreAmorphusTrack*>(workerScorer);
...
void MyScoreAmorphusTrack::UserHookForEndOfRun()
{
...
void MyScoreAmorphusTrack::FillHisto(G4double data, std::vector<G4double> &hist)
{
...
std::vector<G4double> MyScoreAmorphusTrack::Interpolate(std::vector<std::vector<G4double>> Data, G4double x, G4bool extrapolate )
{
...
void MyScoreAmorphusTrack::GetLUT(std::unordered_map<std::string, std::vector<std::vector<G4double>> > & lut, std::string lutfolder, std::string ztype)
{
...
void MyScoreAmorphusTrack::Output()
{
